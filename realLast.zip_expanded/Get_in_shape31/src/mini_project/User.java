...
public class User implements Color {
   ...
   public void openTrainingLog() {
      ...
      boolean status = true;
      List<TrainingDiaryVO> u = da.getTrainingData();
      for (int i = 0; i < u.size(); i++) {

         TrainingDiaryVO user = u.get(i);

         if (user.getUserId().equals(now_id)) {
            status = false;
            System.out.println("--------------------------------------------------------------");
            System.out.println("\t[" + progress + "번째 훈련일지]\n");
            System.out.printf("▶ 운동 날짜 : " + user.getToDate()+"\n");
            System.out.printf("▶ 운동 종류 : " + user.getWorkoutName() + " \t| ▶ 운동 부위 : " + user.getBodyPart()+"\n");
            System.out.printf("▶ 운동 시간 : %.1f분"+"\t| ▶ 운동 난이도 : %s", user.getWorkoutTime(), user.getDifficulty()+"\n");
            System.out.println("▶ 의견 : " + user.getToFeedback());
            System.out.println("--------------------------------------------------------------");
